...
RSpec.describe User, type: :model do
  describe 'validation' do
    context 'when valid' do
      ...
      it 'is valid with another name' do
        user = create(:user)
    ...
    context 'when invalid' do
      ...
      it 'is invalid with not same password & password_confirmation' do
        user_with_not_same_password_and_password_confirmation = build(:user, password: 'password', password_confirmation: 'password123456')
        expect(user_with_not_same_password_and_password_confirmation).to be_invalid
        expect(user_with_not_same_password_and_password_confirmation.errors[:password_confirmation]).to include('とパスワードの入力が一致しません')
      end
    end
  end
end
